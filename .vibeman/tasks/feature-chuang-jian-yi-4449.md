...
- [x] 页面提供双栏布局：左侧为可编辑输入区，右侧为格式化输出区；两侧同时显示同一段文案的原始与修正结果。
- [x] 系统依据《中文文案排版指北》规则对输入文本执行检查与修正，至少覆盖中英文/数字间空格、常见中英文标点规范、全角半角混用三类规则。
- [x] 输出区对每一处修正提供可视化高亮，且可区分“已修正文本片段”与“未变更文本片段”。
- [x] 每一处高亮可追溯到对应规则信息（规则名或说明），用户可以理解该处被修改的原因。
- [x] 当输入文本已符合规则时，系统明确展示“无排版问题”状态，且输出与输入完全一致。
- [x] 当输入为空、仅空白字符或仅换行时，系统不报错；输出为空并保持页面可交互。
- [x] 系统正确处理多行文本，不丢失原有换行结构；URL、邮箱、代码片段样式文本不被误改。
- [x] 提供纠错结果统计信息，至少包含总问题数与按规则分类计数。
- [x] 核心规则引擎具备自动化测试，覆盖正常场景与边界场景；关键前端交互具备端到端或集成测试验证。
- [x] 项目在本地可通过统一命令完成静态检查、测试与构建，且全部通过。
...
- Step 1: 范围冻结与样例集建立
  - [x] 明确首版规则清单与优先级，形成可追踪规则表（规则 id、说明、输入示例、期望输出）。
  - [x] 从《中文文案排版指北》整理最小可用测试语料，覆盖正例、反例与边界案例。
  - [x] 定义统一诊断数据结构与输出契约，确保后续规则和 UI 使用同一接口。
- Step 2: 规则引擎与修正流水线
  - [x] 实现文本检测与自动修正流水线，支持一次执行返回“修正后文本 + 诊断列表 + 统计信息”。
  - [x] 完成规则冲突处理策略（执行顺序、重复命中去重、位置回写）并用测试固定行为。
  - [x] 为每条核心规则补齐单元测试，覆盖多行文本、空输入、URL/邮箱不误改等边界。
- Step 3: 双栏界面与高亮呈现
  - [x] 实现左输入右输出布局，支持实时或手动触发纠错并稳定展示结果。
  - [x] 接入高亮渲染与规则说明展示，保证每个高亮片段可追溯到规则信息。
  - [x] 增加无问题状态、空输入状态与错误回退状态，确保界面行为可预期。
- Step 4: 质量门禁与交付
  - [x] 增加关键流程的集成或 E2E 测试（输入示例 -> 输出文本 -> 高亮与统计一致）。
  - [x] 验证长文本性能阈值并记录基线（例如 5k 字符在可接受时间内完成检测与渲染）。
  - [x] 补充 README 中的运行与验证说明，确保新成员可一键启动与验收。
...
- 完成 review 反馈修复：实现真正的点击定位（点击高亮或问题项会聚焦输入框并选中对应区间），支持更直观的问题回溯。
- 调整空输入行为：当输入为空、仅空白或仅换行时，规则引擎统一返回空输出，页面保持可交互并明确展示空状态。
- 增强标点规则上下文：支持中文句子中夹英文词的场景（如 `这是 OpenAI!`）进行中文标点规范化。
- 增加可重复的性能基线验证：新增 `tests/perf/perf.test.ts` 与 `npm run test:perf`，覆盖 5k+ 文本并校验处理时间低于 60 秒。
- 同步更新单元/集成/E2E 测试与 README，确保规则行为、定位交互和性能命令均可验证。
